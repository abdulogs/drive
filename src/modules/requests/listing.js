function card(item) {
    return `
    <td class="align-middle text-white border-dark px-3 cursor-pointer detailBtn" data-id="${item.id}">${item.name}</td>
    <td class="align-middle text-white border-dark">${item.email}</td>
    <td class="align-middle text-white border-dark">${item.phone}</td>
    <td class="align-middle text-white border-dark">${is_active(item.is_approved)}</td>
    <td class="align-middle text-white border-dark">${is_active(item.is_active)}</td>
    <td class="align-middle text-white border-dark">${timenow(item.updated_at)}</td>
    ...
    <td class="align-middle text-white border-dark px-3">
        ...
        <a class="btn btn-primary btn-sm font-16 p-1 rounded-circle bx bx-list-ul text-dark historyBtn" data-id="${item.id}" href="javascript:void(0)"></a>
...
function loadData(page = 1) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.count == 0) {
                $('#listing').html(`<tr id="emptyrow"><td colspan="8" class="border-0 text-white">${is_empty("0 Requests found!")}</td><tr>`);
