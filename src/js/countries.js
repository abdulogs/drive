...
countries.sort()
const s_country = $("#country").data("selected");
const s_phonec = $("#phone_code").data("selected");
...
countries.forEach(item => {
    country_template += `<option value='${item.name}' data-code="${item.code}" ${(s_country == item.name) ? "selected" : ""}>${item.name}</option>`
    phone_template += `<option value='${item.phone}' ${(s_phonec == item.phone) ? "selected" : ""}>${item.phone}</option>`
});

$("#country").html(country_template)
$("#phone_code").html(phone_template)